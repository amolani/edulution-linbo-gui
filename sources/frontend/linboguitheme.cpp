...
LinboGuiTheme::LinboGuiTheme(LinboBackend* backend, QMainWindow* mainWindow, QObject *parent) : QObject(parent)
{
    ...
    QString val = qEnvironmentVariable("LINBO_LOW_FX").toLower();
    this->_lowFx = !val.isEmpty() && val != "0" && val != "false";
}

QColor LinboGuiTheme::glassBg() const {
    // Edulution: card-like elevated surface on dark bg
    return QColor(255, 255, 255, 18);
}

QColor LinboGuiTheme::glassBorder() const {
    // Edulution: --border: #333333
    return QColor(51, 51, 51, 255);
}

QColor LinboGuiTheme::glassSubtleBg() const {
    // Edulution: subtle card surface
    return QColor(255, 255, 255, 13);
}

QColor LinboGuiTheme::glassSubtleBorder() const {
    // Edulution: subtle border
    return QColor(51, 51, 51, 200);
}

QColor LinboGuiTheme::textAt(int alpha) const {
    return QColor(255, 255, 255, alpha);
}

bool LinboGuiTheme::lowFxMode() const {
    return this->_lowFx;
}
...
int LinboGuiTheme::size(LinboTheme::SizeRole sizeRole) {
    ...
    case LinboTheme::TopLogoHeight:
        return rootHeight * 0.06; // reduced for more OS card space
    case LinboTheme::BottomLogoHeight:
        return rootHeight * 0.03; // reduced for more OS card space
    }
    return 0;
}
