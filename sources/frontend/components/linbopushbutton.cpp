...
void LinboPushButton::resizeEvent(QResizeEvent *event) {
    ...
    if(this->_label != nullptr && !this->_label->text().isEmpty()) {
        ...
        font.setPixelSize(gTheme->toFontSize(this->height() * 0.45));
    ...
    // Pill buttons: calculate minimum width based on text
    if(this->_pillColor.isValid() && this->_label && !this->_label->text().isEmpty()) {
        QFont font;
        font.setPixelSize(gTheme->toFontSize(this->height() * 0.45));
        font.setBold(true);
        QFontMetrics fm(font);
        int textWidth = fm.horizontalAdvance(this->_label->text());
        int hPad = std::max(8, this->height() / 2);
        this->setMinimumWidth(textWidth + hPad * 2);
    }
}

void LinboPushButton::setPillColor(const QColor& color) {
    this->_pillColor = color;
    // Pill buttons: disable platform background, hide ALL child/overlay widgets
    // (we paint text ourselves via QPainter — child widgets cause ghost text on linuxfb)
    ...
    for(LinboPushButtonOverlay* overlay : this->_overlays) {
        ...
        if(overlay->_widget) {
            overlay->_widget->setVisible(false);
            overlay->_widget->setGeometry(0, 0, 0, 0);
        }
    }
    this->update();
}

void LinboPushButton::setPillGradient(const QColor& start, const QColor& end) {
    this->_pillGradientEnd = end;
    this->setPillColor(start);
}

void LinboPushButton::setGhostPill(bool ghost) {
    this->_ghostPill = ghost;
    this->update();
}
...
void LinboPushButton::paintEvent(QPaintEvent *e) {
    ...
    // Pill buttons: full QPainter rendering (reliable on linuxfb)
    if(this->_pillColor.isValid()) {
        ...
        if(this->_ghostPill) {
            // Ghost pill: border only, subtle fill on hover/press
            QColor borderColor = this->_pillColor;
            borderColor.setAlpha(this->_isHovered ? 160 : 100);
            QColor fillColor = Qt::transparent;
            if(this->_isPressed)
                fillColor = QColor(this->_pillColor.red(), this->_pillColor.green(), this->_pillColor.blue(), 50);
            else if(this->_isHovered)
                fillColor = QColor(this->_pillColor.red(), this->_pillColor.green(), this->_pillColor.blue(), 25);
            ...
            // Text in pill color
            if(this->_label && !this->_label->text().isEmpty()) {
                QFont font;
                font.setPixelSize(gTheme->toFontSize(h * 0.43));
                font.setBold(false);
                painter.setFont(font);
                QColor textColor = this->_pillColor;
                textColor.setAlpha(this->_isHovered ? 230 : 180);
                painter.setPen(textColor);
                painter.drawText(pillRect, Qt::AlignCenter, this->_label->text());
            }
        } else {
            // Solid pill: filled background (gradient or solid)
            painter.setPen(Qt::NoPen);
            if(this->_pillGradientEnd.isValid()) {
                QColor c1 = this->_pillColor;
                QColor c2 = this->_pillGradientEnd;
                if(this->_isPressed) { c1 = c1.darker(120); c2 = c2.darker(120); }
                else if(this->_isHovered) { c1 = c1.lighter(112); c2 = c2.lighter(112); }
                QLinearGradient grad(pillRect.left(), 0, pillRect.right(), 0);
                grad.setColorAt(0.0, c1);
                grad.setColorAt(1.0, c2);
                painter.setBrush(grad);
            } else {
                QColor bgColor = this->_pillColor;
                if(this->_isPressed)
            ...
            if(this->_label && !this->_label->text().isEmpty()) {
                QFont font;
                font.setPixelSize(gTheme->toFontSize(h * 0.45));
                font.setBold(true);
                painter.setFont(font);
        ...
        return; // skip overlay system for pill buttons
    }
