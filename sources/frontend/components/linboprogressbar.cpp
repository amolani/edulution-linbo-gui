...
void LinboProgressBar::_paint(QPaintEvent* e, _FromTo values) {
    ...
    int radius = e->rect().height() / 2;

    // glass track background
    painter.setPen(Qt::NoPen);
    painter.setBrush(gTheme->glassBg());
    painter.drawRoundedRect(e->rect(), radius, radius);

    // progress fill
    int fillX = e->rect().width() * values.from;
    int fillW = e->rect().width() * values.to - fillX;
    if(fillW > 0) {
        QRect fillRect(fillX, 0, fillW, e->rect().height());
        painter.setBrush(gTheme->color(LinboTheme::AccentColor));
