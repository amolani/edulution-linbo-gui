...
void LinboDialog::_initColors() {
    ...
    this->setPalette(pal);
    this->setStyleSheet(gTheme->insertValues(
        QString("QLabel{color: %TextColor;} "
                "LinboDialog { border: 1px solid %1; }")
            .arg(QColor(51, 51, 51, 255).name(QColor::HexArgb))));
}
...
void LinboDialog::_initToolbar() {
    ...
    QPalette toolBarPal = this->palette();
    toolBarPal.setColor(QPalette::Window, QColor("#111111"));
    this->_toolBarWidget->setPalette(toolBarPal);
...
void LinboDialog::paintEvent(QPaintEvent *e) {
    ...
    // Explicit solid background (linuxfb palette rendering unreliable)
